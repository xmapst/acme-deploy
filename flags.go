...
func createFlags() []cli.Flag {
	return []cli.Flag{
		...
		cli.StringFlag{
			Name:  "deploy",
			Usage: "Key type to use for deploy type. Supported: secret, kong, nginx. Run 'acme deploy' for help on usage.",
		},
		cli.StringFlag{
			Name:  "dns",
			Usage: "Solve a DNS challenge using the specified provider. Can be mixed with other types of challenges. Run 'acme dns' for help on usage.",
		},
		cli.BoolFlag{
			Name:  "dns.disable-cp",
...
func displayDeployHelp(name string) error {
	w := tabwriter.NewWriter(os.Stdout, 0, 0, 2, ' ', 0)
	ew := &errWriter{w: w}
	switch name {
	case "secret":
		ew.writeln(`Configuration for k8s secret.`)
		ew.writeln(`Code:	'secret'`)
		ew.writeln(`Note: `)
		ew.writeln(`	Default use token and ca`)
		ew.writeln(`	/var/run/secrets/kubernetes.io/serviceaccount/token`)
		ew.writeln(`	/var/run/secrets/kubernetes.io/serviceaccount/ca.crt`)
		ew.writeln()
		ew.writeln(`	- "KUBECONF":	kubernetes config (Optional)`)
		ew.writeln(`	- "NAMESPACE":	kubernetes secret namespace`)
		ew.writeln(`	- "SECRETNAME":	kubernetes secret name`)
		ew.writeln(`	- "CERT":	kubernetes secret CERT name. default cert`)
		ew.writeln(`	- "KEY":	kubernetes secret KEY name. default key`)
	case "kong":
		ew.writeln(`Configuration for Kong.`)
		ew.writeln(`Code:	'kong'`)
		ew.writeln(`	- "KONG_ADMIN_ADDR":	kong admin api url`)
	case "nginx":
		ew.writeln(`Configuration for Nginx.`)
		ew.writeln(`Code:	'nginx'`)
		ew.writeln(`	- "CERT_PATH":	Configure NGINX SSL certificate location`)
		ew.writeln(`	- "KEY_PATH":	Configure NGINX SSL key location`)
	default:
		return fmt.Errorf("%q is not yet supported", name)
	}
